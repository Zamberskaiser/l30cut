...
- [ ] 6. Assistente: cartão de plano humano-primeiro, Pré-visualizar (diff sem mutação), Ajustar, Descartar
- [ ] 7. Testes novos (atalhos, reducer, geometria/gestos, schema v1, treinamento) + typecheck + lint + build

## Review final (aplicar integralmente)

- [ ] R1. TimelineRuler: coleção única e determinística de ticks (sem arrays fragmentados)
- [ ] R2. Nenhum controle decorativo no header/toolbar (não existe "Transitions"; auditar todos os botões)
- [ ] R3. Dialogs (Atalhos/Confirmar/Export) com altura/scroll corretos em telas menores
- [ ] R4. CommandPalette: composição de Dialog válida + abrir/pesquisar/executar/fechar por teclado
- [ ] R5. A11y: remover role="slider" indevido da régua; rótulos de Track Select = selecionar, não mover
- [ ] R6. Fronteira de segurança nativa: validação Rust allowlisted p/ operações de IA; remover plugin de shell;
      deixar explícito o que ainda é contrato
- [ ] R7. Mute de trilha real (comando setTrackMute) — nenhum ícone sem comportamento
- [ ] R8. Executar npm run types / test -- --run / lint -- --quiet / build e corrigir falhas
